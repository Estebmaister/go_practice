...
var _ = Describe("MonsterController", func() {
	...
	Describe("Import CSV", func() {
		...
		JustBeforeEach(func() {
			...
			file, _ := os.Open(cwd + fileName)
			defer file.Close()
			part2, _ := writer.CreateFormFile("file", filepath.Base(cwd+fileName))
			io.Copy(part2, file)
			writer.Close()
			req, _ := http.NewRequest(http.MethodPost, "/monsters/import", payload)
		...
		Context("should fail when importing csv file with an empty monster", func() {
			...
			BeforeEach(func() {
				fileName = "/data/monsters-empty-monster.csv"
			})
		...
		Context("should fail when importing csv file with wrong or inexistent columns.", func() {
			...
			BeforeEach(func() {
				fileName = "/data/monsters-wrong-column.csv"
			})
		...
		Context("should import all the CSV objects into the database successfully", func() {
			...
			BeforeEach(func() {
				fileName = "/data/monsters-correct.csv"
			})
