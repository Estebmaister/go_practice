...
func ListMonsters(context *gin.Context) {
	...
	log.Printf("Found %v monsters", result.RowsAffected)
...
func ImportCSV(context *gin.Context) {
	...
	if csvErr != nil || len(records) == 0 {
		context.AbortWithStatus(http.StatusBadRequest)
	}

	columnNames := []string{"name", "attack", "defense", "hp", "speed", "imageUrl"}

	for idx, row := range records {
		if idx == 0 && !reflect.DeepEqual(columnNames, row) {
			context.AbortWithStatus(http.StatusBadRequest)
